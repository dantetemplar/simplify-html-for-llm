...
def is_tailwind_class(class_name: str) -> bool:
    ...
    common_semantic_classes = [
        'container', 'wrapper', 'section', 'content', 'header', 'footer', 'sidebar',
        'nav', 'main', 'row', 'column', 'box', 'card', 'btn', 'button', 'input',
        'form', 'modal', 'dropdown', 'menu', 'list', 'item', 'page', 'panel',
        'content-section', 'custom-class', 'my-wrapper'
    ]
    ...
    if (class_name.endswith('-section') or class_name.endswith('-container') or 
        class_name.endswith('-wrapper') or class_name.startswith('custom-') or
        class_name.startswith('js-') or class_name.startswith('my-')):
        return False
